...
\begin{prop}[title=Stochastic Gradient Descent (SGD)]
Momentum: Accelerates consistent directions and damps oscillations.
\[
...
\begin{defn}[title=Goal]
Given \(x_i\in\R^D\), find \(y_i\in\R^d\), \(d<D\), preserving useful structure. Reduces cost and can regularize.
\end{defn}
...
\]
Pick eigenvectors of \(C\) with largest eigenvalues. Projected variance equals eigenvalue.
\end{prop}
...
\]
PCA minimizes reconstruction error among linear orthogonal projections. Keeping all \(D\) components loses no information and decorrelates features. Keep \(d\) such that:
\[
\sum_{j=1}^{d}\lambda_j\ge0.9\sum_{k=1}^{D}\lambda_k
\]
\end{prop}

\begin{warn}[title=PCA Tricks]
PCA is unsupervised: labels do not affect components. It is sensitive to feature scale; normalize/standardize first. Principal components are ordered by eigenvalues, not eigenvector magnitudes. PCA can reduce noise and be seen as a linear autoencoder.
\end{warn}
...
\[
\tilde K=K-\mathbf1_NK-K\mathbf1_N+\mathbf1_NK\mathbf1_N
\]
...
\begin{center} % Peut être dans une autre section
...
\begin{prop}[title=Connectivity Instead of Compactness]
...
Logistic regression is a classifier, not regression.\\
Cross-entropy is differentiable and trainable by GD.\\
...
Self-attention parallelizes sequence processing unlike recurrent encoders.
\end{warn}
