...
$\Theta(n^2)$ worst case. $\Theta(n)$ best case.
\end{prop}

\begin{thm}[title=Loop invariant]
Before iter.\ $j$: $A[1..j{-}1]$ sorted (original elems).
\end{thm}

\begin{prop}[title=How to prove it (similar to induction)]
\textbf{Init:} true before 1st iter. \textbf{Maint:} true before iter $\Rightarrow$ true before next. \textbf{Term:} invariant + loop end
\end{prop}
