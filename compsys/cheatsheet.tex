...
\begin{prop}[title=\texttt{open()} flags]
    \begin{tabular}{ll}
...
\begin{defn}[title=File Descriptor Table]
    Per-process table caching the result of \texttt{open()} (path walk done once). Fixed entries: \textbf{0}=STDIN, \textbf{1}=STDOUT, \textbf{2}=STDERR. Each entry stores \textbf{inode}, \textbf{device}, \textbf{offset}. Each \texttt{open()} creates an independent entry with its own offset.
\end{defn}
...
\begin{defn}[title=Disk Layout]
    Boot block $\to$ Superblock $\to$ [data bitmap + inode bitmap] $\to$ Inodes $\to$ Data.
\end{defn}
