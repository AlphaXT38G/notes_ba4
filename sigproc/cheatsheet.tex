...
\begin{thm}[title=Rappel de proba +]
\begin{itemize}
  \item \textbf{Auto-correlation} $R_{X}=E[x^2]$
  \item \textbf{Cross-correlation} $R_{XY}=E[xy]$
  \item \textbf{Covariance} $C_{XY}=E[(x-m_x)(y-m_y)]$
  \item \textbf{if zero-mean} $C_{XY}=R_{XY}$
  \item \textbf{Uncorrelated} $E_{XY}=E[x]E[y]$
  \item \textbf{Independent} $f_{XY}(x,y)=f_{X}(x)f_{Y}(y)$
\end{itemize}
\end{thm}
